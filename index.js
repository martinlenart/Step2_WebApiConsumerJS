...
(async () => {
  ...
  data = await myFetch(`${urlFriends}?count=5`);
  if (data) {
  ...
  let friend = await myFetch(`${urlFriends}/${data[0].friendID}`);
  if (friend) {
  ...
  data = await myFetch(`${urlFriends}/${friend.friendID}`, 'PUT', friend);
  if (data) {
  ...
  await myFetch(`${urlFriends}/${friend.friendID}`, 'DELETE');
  ...
  data = await myFetch(urlFriends, 'POST', friend);
  if (data) {
  ...
  //Test GET api/friends?count=5
  console.log("Test GET all quotes and list the 5 first");
  data = await myFetch(`${urlQuotes}?count=5`);
  if (data) {
